...
int test_logb(void* handle) {
    ...
    float (*lamd_logbf)(float);
    double (*lamd_logb)(double);

    /*
    __m128d (*lamd_vrd2_logb)  (__m128d);
    __m128  (*lamd_vrs4_logbf) (__m128);
    __m256d (*lamd_vrd4_logb)  (__m256d);
    __m256  (*lamd_vrs8_logbf) (__m256);
    */
...
*/
    /*scalar routines*/
    lamd_logbf = dlsym(handle, "amd_logbf");
    lamd_logb  = dlsym(handle, "amd_logb");
    /*vector routines*/
/*
    lamd_vrd2_logb  = dlsym(handle, "amd_vrd2_logb");
    lamd_vrs4_logbf = dlsym(handle, "amd_vrs4_logbf");
    lamd_vrd4_logb  = dlsym(handle, "amd_vrd4_logb");
    lamd_vrs8_logbf = dlsym(handle, "amd_vrs8_logbf");
*/
    error = dlerror();
    if (error != NULL) {
        printf("Error: %s\n", error);
...
*/
    /*vrs8*/
/*
    op_vrs8 = (*lamd_vrs8_logbf)(ip_vrs8);
    _mm256_storeu_ps(output_array_vrs8, op_vrs8);
    printf("amd_vrs8_logbf\ninput:");
    for(i=0; i<8; i++)
