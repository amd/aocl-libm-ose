...
int test_acos(void* handle) {
    ...
    const char* func_name = "acos";
        ...
        data.v8s = (funcf_v8s)GetProcAddress(handle, "amd_vrs8_acosf");
    #else
        data.s1f = (funcf)dlsym(handle, "amd_acosf");
        data.s1d = (func)dlsym(handle, "amd_acos");
        data.v4s = (funcf_v4s)dlsym(handle, "amd_vrs4_acosf");
        data.v8s = (funcf_v8s)dlsym(handle, "amd_vrs8_acosf");
    #endif
    if (data.s1f == NULL || data.s1d == NULL || data.v4s == NULL || data.v8s == NULL) {
        ret = 1;
    }
    if (ret == 1) {
