...
fname PROC FRAME
    ...
    punpckldq  xmm1,xmm0  ;xmm1 = XX,XX,x,y
...
L__compute_logx:
    ...
    pshufd      xmm2,xmm0,238       ;xmm2 = x,x 
    pand        xmm2, XMMWORD PTR L__xexp_bits_xmantissa_bits   ;xmm2 = xexp_bits, xmantissa_bits  
    movd        rcx,xmm2
    shr         rcx,44
    adc         ecx,0
    mov         eax,ecx
    ...
    shl         rcx,44
    movd        xmm1,rcx ;F
    por         xmm2, XMMWORD PTR L__1_by_2 ;xexp_bits, Y
    pshufd      xmm3,xmm2,238       ;xmm3 = xexp_bits,xexp_bits
    psrlq       xmm3,52
    ...
    subsd     xmm1,xmm2    ;xmm1 = f
    
    ;r = f * L__log_F_inv[index]
    mulsd     xmm1, QWORD PTR [rcx+rax*8]     ;xmm1 = XX,r
    movapd    xmm2,xmm1    ;xmm2 = r
    
    ;q = poly = r + r*r*(1/2 + 1/3 * r);
    movsd       xmm4,L__1_by_3
    mulsd       xmm4,xmm1   ;xmm4 = 1/3 * r
    mulsd       xmm1,xmm2   ;xmm1 =  r  * r
    addsd       xmm4,L__1_by_2 ;xmm4 = 1/2 + (1/3 * r)
    mulsd       xmm1,xmm4  ;xmm1 = r*r*(1/2 + (1/3 * r))
    addsd       xmm1,xmm2  ;xmm1 = XX,q=poly = r+r*r*(1/2 + (1/3 * r))   
    
    ;logx = L__log2 * m + L__log_256_table[index] - poly;

    mulsd     xmm3,L__log2           ;xmm3 = L__log2 * m
    lea       rcx, QWORD PTR L__log_256_table
    addsd     xmm3, QWORD PTR [rcx+rax*8]    ;xmm3 = logx 
    subsd     xmm3,xmm1

L__return_from_near_one:    
    ;ylogx = ((double)y) * logx;
    mulsd    xmm0,xmm3     ;xmm0 = ylogx
    ...
    mulpd       xmm3,L__real_64_by_log2   ;xmm3 = x * (64/ln(2)
    ...
    ; q = r+r*r*(1/2 + (1/6 * r))
    movsd       xmm3,L__1_by_6 
    mulsd       xmm3,xmm0  ;xmm3 = 1/6 * r
    mulsd       xmm0,xmm1  ;xmm0 =  r  * r
    addsd       xmm3,L__1_by_2  ;xmm3 = 1/2 + (1/6 * r)
    ...
    paddq       xmm4,xmm0
    cvtsd2ss    xmm0,xmm4
    orps        xmm0,xmm5  ;negate the result if x negative 
...
L__x_is_neg_or_zero_or_near_one:
    ...
    movss xmm5, DWORD PTR L__neg_zero ;set the sign bit
...
L__x_near_one:
    ...
    pshufd      xmm3,xmm0,238 ;xmm3 = double x,x
    
    ; r = x - 1.0
    subsd       xmm3,L__real_one    ; r
    ...
    movapd      xmm7,L__real_ca42
    movapd      xmm8,L__real_ca31   
    movapd       xmm1,xmm3       ; r
    movapd       xmm6,xmm3       ; r
    addsd        xmm1,L__real_two; 2+r
    divsd        xmm6,xmm1        ; r/(2+r) = u/2    
    movapd       xmm2,xmm6        ; u/2
    mulsd        xmm6,xmm3        ; r*u/2 = correction
    addsd        xmm2,xmm2        ; u
    movapd       xmm1,xmm2        ; u
    mulsd        xmm1,xmm2        ; u^2
    pshufd      xmm4,xmm1,68 ; u^2,u^2    

    mulpd       xmm4,xmm7                ; u^2 * ca4,        u^2 * ca2
    mulsd       xmm2,xmm1                ; u^3    
    addpd       xmm4,xmm8                ; u^2 * ca4 + ca3,  u^2 * ca2 + ca1
    mulsd       xmm1,xmm1                ; u^4
    mulsd       xmm1,xmm2                ; u^7
    unpcklpd    xmm2,xmm1                ; u^7, u^3
    mulpd       xmm2,xmm4                ; u^7 * (u^2 * ca4 + ca3),  u^3 * (u^2 * ca2 + ca1)

    pshufd      xmm1,xmm2,238            ; XX,u^7 * (u^2 * ca4 + ca3)    

    addsd       xmm1,xmm2                 ; u^7 * (u^2 * ca4 + ca3) + u^3 * (u^2 * ca2 + ca1)
    subsd       xmm2,xmm6                 ; u^7 * (u^2 * ca4 + ca3) + u^3 * (u^2 * ca2 + ca1) - r*u/2    
    addsd       xmm3,xmm2                 ; r + u^7 * (u^2 * ca4 + ca3) + u^3 * (u^2 * ca2 + ca1) - r*u/2    
...
L__y_is_nan:
    ...
    movdqa xmm0,xmm1     ;;pow(any x other than 1, nan) = qnan
    jmp L__return_nan
...
L__return_x_is_one_y_is_pninf:
    ...
    orpd     xmm1, XMMWORD PTR L__clear_sign
    andpd     xmm0,xmm1     
    movdqa     xmm6 , XMMWORD PTR [save_xmm6+rsp]  
    movdqa     xmm7,  XMMWORD PTR [save_xmm7+rsp]
    add         rsp, stack_size
