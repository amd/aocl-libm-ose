...
float FN_PROTOTYPE(logbf)(float x)
{
  ...
  if ((ux & ~SIGNBIT_SP32) == 0)
	...
	return _amd_handle_errorf("logbf", _FpCodeLogb, NINFBITPATT_SP32, _SING, AMD_F_DIVBYZERO, ERANGE, x, 0.0, 1);
...
#ifdef WINDOWS
          return _amd_handle_errorf("logbf", _FpCodeLogb, ux|0x00400000, DOMAIN, AMD_F_NONE, EDOM, x, 0.0, 1);
#else
          return x+x;
#endif      
      }
...
weak_alias (logbf, FN_PROTOTYPE(logbf))
