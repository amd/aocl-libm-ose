...
long int FN_PROTOTYPE(lround)(double d)
{
    ...
    if ((u64d.u32[1] & 0X7FF00000) == 0x7FF00000)
    {
        ...
        //Raise range or domain error
        #ifdef WIN64
		    _amd_handle_error("lround", _FpCodeLround, SIGNBIT_SP32, DOMAIN, AMD_F_NONE, EDOM, d, 0.0, 1);
         ...
         if((u64d.u64 & 0x7fffffffffffffff) >= 0x7ff8000000000000)
                _amd_handle_error("lround", _FpCodeLround, (unsigned long long)SIGNBIT_DP64, DOMAIN, AMD_F_NONE, EDOM, d, 0.0, 1);
         else    
                _amd_handle_error("lround", _FpCodeLround, (unsigned long long)SIGNBIT_DP64, DOMAIN, AMD_F_INVALID, EDOM, d, 0.0, 1);
    ...
    if (intexp >= 31)
    {
        ...
        _amd_handle_error("lround", _FpCodeLround, result, DOMAIN, AMD_F_NONE, EDOM, d, 0.0, 1);
        return result;
    }
...
weak_alias (lround, FN_PROTOTYPE(lround))
