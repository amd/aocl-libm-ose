...
double FN_PROTOTYPE(scalbln)(double x, long int n)
{
    ...
    if (val.u64 > 0x7ff0000000000000)     /* x is NaN */
        #ifdef WINDOWS
    	   return _amd_handle_error("scalbn", _FpCodeScalbn, val_x.u64|0x0008000000000000, _DOMAIN, 0, EDOM, x, n, 2);
        #else
           {
           if(!(val.u64 & 0x0008000000000000))// x is snan
                   return _amd_handle_error("scalbn", _FpCodeScalbn, val_x.u64|0x0008000000000000, _DOMAIN, AMD_F_INVALID, EDOM, x, n, 2);
            else
                   return x;
		    }
        #endif
    ...
    if(exponent == 0)/*x is denormal*/
    {
		...
		if(exponent < -MULTIPLIER_DP)/*underflow*/
		{
			...
			return _amd_handle_error("scalbln", _FpCodeScalbln, val.u64, _UNDERFLOW, AMD_F_INEXACT|AMD_F_UNDERFLOW, ERANGE, x, (double)n, 2);
		}
		if(exponent > 2046)/*overflow*/
		{
			val.u32[1] = sign | 0x7ff00000;
			val.u32[0] = 0x00000000;
            return _amd_handle_error("scalbln", _FpCodeScalbln, val.u64, _OVERFLOW, AMD_F_INEXACT|AMD_F_OVERFLOW, ERANGE, x, (double) n, 2);
		}
