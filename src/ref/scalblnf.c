...
float FN_PROTOTYPE(scalblnf)(float x, long int n)
{
    ...
    if(val.u32 > 0x7f800000)/* x= nan*/
        #ifdef WINDOWS
        return _amd_handle_errorf("scalblnf", _FpCodeScalbln, val_x.u32|0x00400000, _DOMAIN, 0, EDOM, x, (float)n, 2);
        #else
        {
          if(!(val.u32 & 0x00400000)) //x is snan
              return _amd_handle_errorf("scalblnf", _FpCodeScalbln, val_x.u32|0x00400000, _DOMAIN, AMD_F_INVALID, EDOM, x, (float)n, 2);
          else
              return x;
		}
        #endif
	...
	if(exponent == 0)/*x is denormal*/
	{
		...
		if(exponent < -MULTIPLIER_SP)/*underflow*/
		{
            ...
            return _amd_handle_errorf("scalblnf", _FpCodeScalbln, val.u32, _UNDERFLOW, AMD_F_INEXACT|AMD_F_UNDERFLOW, ERANGE, x, (float)n, 2);
		}
		if(exponent > 254)/*overflow*/
		{
			val.u32 = sign | 0x7f800000;
            return _amd_handle_errorf("scalblnf", _FpCodeScalbln, val.u32, _OVERFLOW, AMD_F_INEXACT|AMD_F_OVERFLOW, ERANGE, x, (float)n, 2);
		}
