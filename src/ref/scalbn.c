...
double FN_PROTOTYPE(scalbn)(double x, int n)
{
    ...
    if(exponent == 0)/*x is denormal*/
    {
		...
		if(exponent < -MULTIPLIER_DP)/*underflow*/
		{
			...
			return _amd_handle_error("scalbn", _FpCodeScalbn, val.u64, _UNDERFLOW, AMD_F_INEXACT|AMD_F_UNDERFLOW, ERANGE, x, (double)n, 2);
		}
		if(exponent > 2046)/*overflow*/
		{
			val.u32[1] = sign | 0x7ff00000;
			val.u32[0] = 0x00000000;
            return _amd_handle_error("scalbn", _FpCodeScalbn, val.u64, _OVERFLOW, AMD_F_INEXACT|AMD_F_OVERFLOW, ERANGE, x, (double) n, 2);
		}
