...
int test_cos(void* handle) {
    ...
    /*scalar routines*/
    *(void **) (&amd_cosf) = dlsym(handle, "amd_cosf");
    *(void **) (&amd_cos)  = dlsym(handle, "amd_cos");
    /*vector routines*/
    *(void **) (&amd_vrd2_cos)  = dlsym(handle, "amd_vrd2_cos");
    *(void **) (&amd_vrs4_cosf) = dlsym(handle, "amd_vrs4_cosf");
    //*(void **) (&amd_vrd4_cos)  = dlsym(handle, "amd_vrd4_cos");
    //*(void **) (&amd_vrs8_cosf) = dlsym(handle, "amd_vrs8_cosf");
    ...
    printf("Exercisig cos routines\n");

    /*scalar*/
    outputf = (*amd_cosf)(inputf);
    printf("amd_cosf(%f) = %f\n", inputf, outputf);
    output = (*amd_cos)(input);
    printf("amd_cos(%lf) = %lf\n", input, output);

    /*vrd2*/
    op_vrd2 = (*amd_vrd2_cos)(ip_vrd2);
    ...
    */
    ...
    printf("\nOutput:\n");
    for(i=0; i<8; i++)
      b  printf("%f\t",output_array_vrs8[i]);
    */
    printf("\n");
