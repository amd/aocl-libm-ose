...
#else
	{0x3FF0000000000000, 0x3fe14a280fb5068c, 0x3feaed548f090cee},  // 1
#endif
...
#else
	{0x3ff921fb57442d18, 0xbe47fffffdcb3b39, 0x3FEFFFFFFFFFFFFF},  // 1.5707963379707675 close to pi/2
    {0xbff921fb57442d18, 0xbe47fffffdcb3b39, 0xBFEFFFFFFFFFFFFF},  //-1.5707963379707675 close to - pi/2
#endif
...
extern "C" double S_SP_COS(double);
...
void acc_fastcos()
{
    ...
    for(k=0; k<LENGTH_OF(tcos_sin); k++)
    {
        v_x.u64 = pTest[k].ux;
        doubleword output;
        output.u64=pTest[k].ucos;
        
        v_cos.f64 = S_SP_COS(v_x.f64);

		if(v_cos.u64 != pTest[k].ucos && !(isnan(v_cos.f64) && isnan(output.f64))) 
		{	
			flag++;

            std::cout << "Failure: " << std::dec << k << " , expected: " << std::hex << 
                pTest[k].ucos << ", Actual: " << v_cos.u64 << std::endl;
			//v_cos.f64 = cos(v_x.f64);
			//std::cout << "\t The real answer is 0x" << std::hex << v_cos.u64 << std::endl;
        }
    }
