...
{			                //cos	           //sin
	...
	// MS and Gcc answer for the following test case 0x3FE67D4903528614
...
extern "C" double S_SP_SIN(double);
 
void acc_fastsin()
{
    const TESTPAIR *pTest = tcos_sin;
	Hold64 v_x, v_sin,output;
	int k, flag=0;
    ...
    for(k=0; k<LENGTH_OF(tcos_sin); k++)
    {
        ...
        v_sin.f64 = S_SP_SIN(v_x.f64);
          
        output.u64 =  pTest[k].usin; 

		if(v_sin.u64 != pTest[k].usin && !(isnan(v_sin.f64) && isnan(output.f64))) 
		{	
			flag++;

            std::cout << "Failure: " << std::dec << k << " , expected: " << std::hex << 
                pTest[k].usin << ", Actual: " << v_sin.u64 << std::endl;
        }
    }
	
	if (flag==0) 	std::cout << "\t" << k << " test cases passed!!!" << std::endl;
	else	std::cout << "\t" << flag <<" cases fail" << std::endl;
