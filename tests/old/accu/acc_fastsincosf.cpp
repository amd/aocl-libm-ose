...
#define S_SP_SINCOS sincosf
#else
#define S_SP_SINCOS sincosf
//#define S_SP_SINCOS fastsincosf
#endif
...
{			     //cos	     //sin
	...
	//exceptional cases
#ifdef WIN64 // windows does not support sincos function
...
extern "C" void S_SP_SINCOS(float x, float* sinfx, float* cosfx);
 
void acc_fastsincosf()
{
    ...
    for(k=0; k<LENGTH_OF(tcos_sinf); k++)
    {
        ...
        S_SP_SINCOS(v_x.f32, &(v_sin.f32), &(v_cos.f32));

        if ((v_sin.u32 != pTest[k].usin) || (v_cos.u32 != pTest[k].ucos)) 
        {
			flag++;
            std::cout << "Failure: " << std::dec << k << " , expected: (" << std::hex << 
                pTest[k].usin << ", " << pTest[k].ucos << "), Actual: (" << v_sin.u32 << ", " << v_cos.u32 
				<< ")" << std::endl;
        }
