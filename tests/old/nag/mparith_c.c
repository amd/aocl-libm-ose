...
static char *dectobin(int k, int bits)
{
...
void MPSQRT(int *XMP, int *YMP, fp_params PARAMS, int *RESULT, int *IFAIL)
{
  ...
  if (!IWORK || !logbase || !log10e)
  ...
  else
    {
      ...
      if (mpisnan(XMP, &PARAMS.n, PARAMS.params) || mpisneg(XMP, &PARAMS.n, PARAMS.params))
      {
...
void MPCEXP(int *XMP, int *YMP, fp_params PARAMS, int *RESULT_RE, int *RESULT_IM, int *IFAIL)
{
    ...
    MPSINCOSTAN(YMP, PARAMS, result_sin, result_cos, result_tan, &ifail_sincos); /* sincos */

    times(result_exp,result_cos,RESULT_RE,&PARAMS.n,PARAMS.params,&ifail_re); /* result_real = exp * cos */
    times(result_exp,result_sin,RESULT_IM,&PARAMS.n,PARAMS.params,&ifail_im); /* result_imag = exp * sin */
...
void MP2DEC_R(int *XMP,fp_params PARAMS, int DIGITS, char *C, int lenC, int *IFAIL)
{
...
void GET_PRECOMPUTED_LOG_ETC(fp_params PARAMS,
...
static void splitf(REAL f, int *e, REAL *m)
{
  int expont, i;
  REAL y;
  static int first = 1;
                           ...
                           const char *expected)
{
  ...
  if (verbose > 0)
    {
      fprintf(verboseSummaryFile, "\n");
      fprintf(verboseSummaryFile, "Testing %s for %d arguments near multiples of pi/4\n",
              funName, nArgs);
...
{
  fprintf(summaryFile, "\n");
  fprintf(summaryFile, "Testing %s for arguments near exact integers in range [%d,%d]\n",
  ...
  if (verbose > 0)
    {
      fprintf(verboseSummaryFile, "\n");
      fprintf(verboseSummaryFile, "Testing %s for arguments near exact integers in range [%d,%d]\n",
              funName, nLow, nLow+nArgs);
      fprintf(verboseSummaryFile, "Maximum relative difference = %g  (%g ulps)\n",
              maxreldiff, maxulps);
      if (maxreldiff > 0.0)
  ...
  if (first)
    {
      ...
      if (verbose > 0)
        {
          fprintf(verboseSummaryFile, "\n");
          fprintf(verboseSummaryFile, "Summary of errors in %s over all tested intervals:\n", funName);
                          ...
                          REAL maxTimeNormal, int flagsOK)
{
  ...
  if (verbose > 0)
    {
      fprintf(verboseSummaryFile, "\n");
      fprintf(verboseSummaryFile, "Test of %s concluded at %s", funName, ctime(&theTime));
...
void parseArgs(int argc, char *argv[], int *nTests, int *nTimes, int *amean, 
