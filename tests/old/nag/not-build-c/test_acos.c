  ...
  if (!openResultsFiles(test_string,
  ...
  else
    {
      ...
      /* Tests of x values approaching 1.0, where we might
         see an increase in relative error for acos(x). */
      a = (REAL)0.999;
      b = 1.0;
      test_interval(params, base, mantis, emin, emax, a, b,
                    nTests, nTimes, amean, overhead, verbose,
                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPACOS(xmp, params, result, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of acos(x):\n");
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time acos(x) in the interval [a,b] */
  ...
  for (i = 0; i < n; i++)
    {
      ...
      if ((y < -0.0) || (y > pi))
        {
          /* Yuk: acos(x) returned something less than 0.0 or bigger than Pi */
          if (!badRangeResult)
            {
              if (y < 0.0)
                {
                ...
                {
                  sprintf(line,
                          "**** WARNING: %s returned result greater than pi ****\n"
                          "     at x  = %65.54e\n"
                          "       %s  = %40.29e\n",
                          FSPECSTR, x, FSPECSTR, y);
                  printf("%s", line);
