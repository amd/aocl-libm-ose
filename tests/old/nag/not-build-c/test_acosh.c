                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPACOSH(xmp, params, result_acosh, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_acosh, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_acosh, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of acosh(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := arccosh(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_acosh, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp, *result_acosh;
  int i, ifail, digs;
  char line[512];
  char *dec;
  REAL cycles, logba;

  /* Time acosh(x) in the interval [a,b] */
