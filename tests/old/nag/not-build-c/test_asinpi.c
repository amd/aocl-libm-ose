  ...
  if (!openResultsFiles(test_string,
  ...
  else
    {
      ...
      /* Tests of x values approaching 1.0, where we might
         see an increase in relative error for asinpi(x). */
      a = (REAL)0.999;
      b = 1.0;
      test_interval(params, base, mantis, emin, emax, a, b,
                    nTests, nTimes, amean, overhead, verbose,
                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPASINPI(xmp, params, result, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of asinpi(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := arcsin(x);\n");
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time asinpi(x) in the interval [a,b] */
  ...
  for (i = 0; i < n; i++)
    {
      ...
      if (my != -y)
        {
          /* Yuk: asinpi(-x) is not equal to -asinpi(x) */
          if (!badSymmetry)
            {
              sprintf(line,
      ...
      if ((y < -0.5) || (y > 0.5))
        {
          /* Yuk: asinpi(x) returned something bigger than 0.5 */
          if (!badRangeResult)
            {
              sprintf(line,
