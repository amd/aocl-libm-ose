                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPATANPI(xmp, params, result, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of atanpi(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := arctan(x);\n");
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time atanpi(x) in the interval [a,b] */
  ...
  for (i = 0; i < n; i++)
    {
      ...
      if (my != -y)
        {
          /* Yuk: atanpi(-x) is not equal to -atanpi(x) */
          if (!badSymmetry)
            {
              sprintf(line,
      ...
      if ((y < -0.5) || (y > 0.5))
        {
          /* Yuk: atanpi(x) returned something bigger than 0.5 */
          if (!badRangeResult)
            {
              sprintf(line,
                      "**** WARNING: %s returned result of magnitude greater than Pi/2 ****\n"
                      "     at x  = %65.54e\n"
                      "       %s  = %40.29e\n",
