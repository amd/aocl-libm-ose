...
{
  REAL y, reldiff, ulps;
  int *xmp, *result_exp, *ymp;//*onemp,*minusonemp,*temp,*temp1;
  int ifail, digs;
  ...
  MPEXPM1(xmp,params,result_exp,&ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_exp, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_exp, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of expm1(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := expm1(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_exp, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp, *result_exp;//,*onemp,*minusonemp,*temp,*temp1;
  int i, ifail, digs;
  char line[512];
  char *dec;
  REAL cycles, logba;
