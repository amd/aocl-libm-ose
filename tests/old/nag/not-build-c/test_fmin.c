                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPFMIN(xmp, ymp, params, result_fmin, &ifail);
  ...
  reldiff = MPRELDIFF(z, base, mantis, emin, emax, result_fmin, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_fmin, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of fmin(x,y):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      mapleX(verboseSummaryFile, y, "y", base, mantis);
      fprintf(verboseSummaryFile, " z := min(x,y);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_fmin, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp, *ymp, *result_fmin;
  int i, j, ifail, digs;
  char line[512];
  char *dec;
  REAL cycles, logba, logdc;

  /* Time fmin(x,y) with x x log-distributed in interval [a,b] and
