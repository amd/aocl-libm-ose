...
int main(int argc, char *argv[])
{
  ...
  if (!openResultsFiles(test_string,
  ...
  else
    {
      ...
      for (i = 10+50*((emax-1)/50+1); i >= 60; i-=50)
        {
          if (i >= emax)
...
{
  REAL y, iptr, reldiff, ulps;
  REAL ulps1, ulps2;
  int *xmp, *result_fract, *ymp, *result_ipart_fract;
  int ifail, digs;
  ...
  xmp = new_mp(params);
  ymp = new_mp(params);
  result_fract = new_mp(params);
  result_ipart_fract = new_mp(params);
  ...
  MPFRACT(xmp, result_fract, result_ipart_fract, params, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_fract, params,
                      &ulps1, &ifail) +
            MPRELDIFF(iptr, base, mantis, emin, emax, result_ipart_fract, params,
                      &ulps2, &ifail);
  ulps = ulps1 + ulps2;
  ...
  dec = MP2DEC(result_fract, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of fract(x,iptr):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " iy := floor(x);\n");
      fprintf(verboseSummaryFile, " y := x - iy;\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_fract, params, mantis, stdout);
    }
...
{
  REAL x, y, iptr, savx, savy, reldiff, ulps, infin, xx, maxreldiff, maxulps;
  REAL ulps1, ulps2;
  int *xmp, *result_fract, *result_ipart_fract;
  int i, ifail, digs;
  char *dec;
...
{
  REAL x = 0.0, y, iptr, reldiff, savx, savy, maxreldiff, ulps, maxulps;
  REAL ulps1, ulps2;
  int *xmp, *result_fract, *result_ipart_fract;
  int i, ifail, digs;
  char line[512];
  ...
  /* Time fract(x,iptr) in the interval [a,b] */

{
  REAL cyc, dt, x=0.0, logba, snormal, iptr;
  volatile REAL y=0.0;
