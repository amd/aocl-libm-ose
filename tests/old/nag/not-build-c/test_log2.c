                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPLOG2(xmp, params, result_log, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of log2(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := log[2](x);\n");
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time log2(x) in the interval [a,b] */
