                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPLOGB(xmp, emin, result_logb, params, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_logb, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_logb, params, digs, &ifail);
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_logb, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time logb(x) in the interval [a,b] */
