...
int main(int argc, char *argv[])
{
  ...
  if (!openResultsFiles(test_string,
  ...
  else
    {
      ...
      createLargest(0, mantis, emax, &x);
      /* Choose elim such that pow10(2.0**elim) ~= large */
                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPPOW10(xmp, params, result_pow10, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_pow10, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_pow10, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of pow10(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := 2.0^x;\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_pow10, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp, *result_pow10;
  int i, ifail, digs;
  char line[512];
  char *dec;
  REAL cycles, logba;

  /* Time pow10(x) in the interval [a,b] */
