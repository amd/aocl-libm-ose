                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPROUND(xmp, result_round, params, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_round, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_round, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of round(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := round(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_round, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp, *result_round;
  int i, ifail, digs;
  char line[512];
  char *dec;
  REAL cycles, logba;

  /* Time round(x) in the interval [a,b] */
