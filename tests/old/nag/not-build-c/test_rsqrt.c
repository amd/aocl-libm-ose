                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPIROOT(xmp, result_rsqrt, params, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_rsqrt, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_rsqrt, params, digs, &ifail);
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_rsqrt, params, mantis, stdout);
    }
