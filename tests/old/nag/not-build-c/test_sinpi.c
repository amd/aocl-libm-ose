                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_sin, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_sin, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      ...
      fprintf(verboseSummaryFile, " y := sinpi(x);\n");
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time sinpi(x) in the interval [a,b] */
  ...
  for (i = 0; i < n; i++)
    {
      ...
      if (my != -y)
        {
          /* Yuk: sinpi(-x) is not equal to -sinpi(x) */
          if (!badSymmetry)
            {
              sprintf(line,
      ...
      if ((y < -1.0) || (y > 1.0))
        {
          /* Yuk: sinpi(x) returned something bigger than 1.0 */
          if (!badRangeResult)
            {
              sprintf(line,
