                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_tan, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_tan, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of tanpi(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := tanpi(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_tan, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time tanpi(x) in the interval [a,b] */
  ...
  for (i = 0; i < n; i++)
    {
      ...
      if (my != -y)
        {
          /* Yuk: tanpi(-x) is not equal to -tanpi(x) */
          if (!badSymmetry)
            {
              sprintf(line,
