  ...
  if (!openResultsFiles(test_string,
  ...
  else
    {
      ...
      /* Tests of x values approaching 1.0, where we might
         see an increase in relative error for asin(x). */
      a = (REAL)0.999;
      b = 1.0;
      test_interval(params, base, mantis, emin, emax, a, b,
                    nTests, nTimes, amean, overhead, verbose,
                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPASIN(xmp, params, result, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of asin(x):\n");
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time asin(x) in the interval [a,b] */
  ...
  for (i = 0; i < n; i++)
    {
      ...
      if (my != -y)
        {
          /* Yuk: asin(-x) is not equal to -asin(x) */
          if (!badSymmetry)
            {
              sprintf(line,
      ...
      if ((y < -piby2) || (y > piby2))
        {
          /* Yuk: asin(x) returned something bigger than Pi/2 */
          if (!badRangeResult)
            {
              sprintf(line,
