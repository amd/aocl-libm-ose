                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPASINH(xmp, params, result_asinh, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_asinh, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_asinh, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of asinh(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := arcsinh(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_asinh, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp, *result_asinh;
  int i, ifail, digs, badSymmetry;
  char line[512];
  char *dec;
  REAL cycles, logba;

  /* Time asinh(x) in the interval [a,b] */
  ...
  for (i = 0; i < n; i++)
    {
      ...
      if (my != -y)
        {
          /* Yuk: asinh(-x) is not equal to -asinh(x) */
          if (!badSymmetry)
            {
              sprintf(line,
