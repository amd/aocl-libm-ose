                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPATAN(xmp, params, result, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of atan(x):\n");
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time atan(x) in the interval [a,b] */
  ...
  for (i = 0; i < n; i++)
    {
      ...
      if (my != -y)
        {
          /* Yuk: atan(-x) is not equal to -atan(x) */
          if (!badSymmetry)
            {
              sprintf(line,
      ...
      if ((y < -piby2) || (y > piby2))
        {
          /* Yuk: atan(x) returned something bigger than Pi/2 */
          if (!badRangeResult)
            {
              sprintf(line,
