                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPATAN2(ymp, xmp, params, result_atan2, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of atan2(x,y):\n");
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time atan2(y,x) with x log-distributed in the interval [a,b] and
      ...
      for (i = 0; i < nX; i++)
        {
          ...
          if ((z < -pi) || (z > pi))
            {
              /* Yuk: atan2(x,y) returned something bigger than Pi */
              if (!badRangeResult)
                {
                  sprintf(line,
