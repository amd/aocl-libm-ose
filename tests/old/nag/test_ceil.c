...
int main(int argc, char *argv[])
{
  ...
  if (!openResultsFiles(test_string,
  ...
  else
    {
      ...
      test_special(params, base, mantis, emin, emax, &x,
                   ...
                   &signZeroOK, verbose, &maxRelErr, &maxUlpErr, &maxX,
                   summaryFile, verboseSummaryFile);
 ...
 conclude:
  ...
  if (!signZeroOK)
    {
      printf("ERROR: sign of zero return result was sometimes incorrect.\n");
      fprintf(summaryFile, "ERROR: sign of zero return result was sometimes incorrect.\n");
      if (verbose)
        fprintf(verboseSummaryFile, "ERROR: sign of zero return result was sometimes incorrect.\n");
    }
...
void test_special(fp_params params, int base, int mantis, int emin, int emax,
                  ...
                  int expectedErrno, int *flagsOK, int *signZeroOK,
                  int verbose, REAL *maxRelErr, REAL *maxUlpErr, REAL *maxX,
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  REAL y, reldiff, ulps;
  int *xmp, *result_ceil, *ymp;
  int ifail, digs;
  ...
  MPCEIL(xmp, result_ceil, params, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_ceil, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_ceil, params, digs, &ifail);
  ...
  /* As a special case, check the sign of the result when it is zero.
     We only do the check once; i.e. if a faulty signed zero has already
     shown up, don't complain again. */
  if (MPISZERO(result_ceil,params) && *signZeroOK != 0)
    {
      int expectedNeg, computedNeg;
      expectedNeg = MPISNEG(result_ceil,params);
      computedNeg = disneg(&y);
      if (expectedNeg != computedNeg)
        {
          *signZeroOK = 0;

          printf("Error: the sign of the zero return value is incorrect.\n");
          fprintf(summaryFile, "Error: the sign of the zero return value is incorrect.\n");
          if (verbose)
            fprintf(verboseSummaryFile, "Error: the sign of the zero return value is incorrect.\n");

          if (expectedNeg)
            {
              printf("Argument x = %g; expected result: -0.0; computed result +0.0\n", x);
              fprintf(summaryFile, "Argument x = %g; expected result: -0.0; computed result +0.0\n", x);
              if (verbose)
                fprintf(verboseSummaryFile, "Argument x = %g; expected result: -0.0; computed result +0.0\n", x);
            }
          else
            {
              printf("Argument x = %g; expected result: +0.0; computed result -0.0\n", x);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of ceil(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := ceil(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_ceil, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time ceil(x) in the interval [a,b] */
