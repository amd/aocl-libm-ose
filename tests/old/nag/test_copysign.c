...
{
  REAL z, reldiff, ulps;
  int *xmp, *ymp, *result_copysign, *zmp;
  int ifail, digs;
  ...
  MPCOPYSIGN(xmp, ymp, result_copysign, params, &ifail);
  ...
  reldiff = MPRELDIFF(z, base, mantis, emin, emax, result_copysign, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_copysign, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of copysign(x,y):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      mapleX(verboseSummaryFile, y, "y", base, mantis);
      fprintf(verboseSummaryFile, " z := abs(x)*signum(y);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_copysign, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp, *ymp, *result_copysign;
  int i, j, ifail, digs, badSymmetry;
  char line[512];
  char *dec;
  REAL cycles, logba, logdc;

  /* Time copysign(x,y) x log-distributed in in the interval [a,b] and
