  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of cos(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := cos(x);\n");
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time cos(x) in the interval [a,b] */
  ...
  for (i = 0; i < n; i++)
    {
      ...
      if (my != y)
        {
          /* Yuk: cos(-x) is not equal to cos(x) */
          if (!badSymmetry)
            {
              sprintf(line,
      ...
      if ((y < -1.0) || (y > 1.0))
        {
          /* Yuk: cos(x) returned something bigger than 1.0 */
          if (!badRangeResult)
            {
              sprintf(line,
