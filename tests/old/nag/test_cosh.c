...
int main(int argc, char *argv[])
{
  ...
  if (!openResultsFiles(test_string,
  ...
  else
    {
      ...
      /* Choose elim such that cosh(2.0**elim) ~= large */
                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPCOSH(xmp, params, result_cosh, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_cosh, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_cosh, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of cosh(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := cosh(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_cosh, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time cosh(x) in the interval [a,b] */
