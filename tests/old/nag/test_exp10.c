...
int main(int argc, char *argv[])
{
  ...
  const REAL log10 = (REAL)2.302585092994045684017991;
  int verbose = 2;
  ...
  if (!openResultsFiles(test_string,
  ...
  else
    {
      ...
      expectedFlags = AMD_F_OVERFLOW | AMD_F_INEXACT;
      ...
      createLargest(0, mantis, emax, &x);
      /* Choose elim such that exp10(2.0**elim) ~= large */
      elim = (int)(mylog(mylog(x)/log10) / log2 + 1);
                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPEXP10(xmp, params, result_exp10, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_exp10, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_exp10, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of exp10(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := 10.0^x;\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_exp10, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp, *result_exp10;
  int i, ifail, digs;
  char line[512];
  char *dec;
  REAL cycles, logba;

  /* Time exp10(x) in the interval [a,b] */
