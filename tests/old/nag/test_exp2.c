...
int main(int argc, char *argv[])
{
  ...
  if (!openResultsFiles(test_string,
  ...
  else
    {
      ...
      createLargest(0, mantis, emax, &x);
      /* Choose elim such that exp2(2.0**elim) ~= large */
      elim = (int)(mylog(mylog(x)/log2) / log2 + 1);
                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPEXP2(xmp, params, result_exp2, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_exp2, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_exp2, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of exp2(x):\n");
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_exp2, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time exp2(x) in the interval [a,b] */
