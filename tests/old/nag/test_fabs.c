                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPFABS(xmp, result_fabs, params, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_fabs, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_fabs, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of sinpi(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := fabs(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_fabs, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time fabs(x) in the interval [a,b] */
