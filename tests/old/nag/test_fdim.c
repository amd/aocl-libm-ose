                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPFDIM(xmp, ymp, params, result_fdim, &ifail);
  ...
  reldiff = MPRELDIFF(z, base, mantis, emin, emax, result_fdim, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_fdim, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of fdim(x,y):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      mapleX(verboseSummaryFile, y, "y", base, mantis);
      fprintf(verboseSummaryFile, " z := max(0.0, x - y);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_fdim, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp, *ymp, *result_fdim;
  int i, j, ifail, digs;
  char line[512];
  char *dec;
  REAL cycles, logba, logdc;

  /* Time fdim(x,y) with x x log-distributed in interval [a,b] and
