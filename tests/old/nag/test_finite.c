...
void test_special(fp_params params,
                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  result_finite = MPFINITE(xmp, params, &ifail);
  ...
  /* We are comparing two boolean values */
  if ((y && !result_finite) || (!y && result_finite))
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp;
  int y, my, result_finite, savy;
  int i, ifail, badSymmetry;
  char line[512];
  REAL cycles, logba;

  /* Time finite(x) in the interval [a,b] */
  {
  ...
  for (i = 0; i < n; i++)
    {
      ...
      if (my != y)
        {
          /* Yuk: finite(-x) is not equal to finite(x) */
          if (!badSymmetry)
            {
              sprintf(line,
