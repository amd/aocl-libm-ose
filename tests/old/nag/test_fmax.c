                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPFMAX(xmp, ymp, params, result_fmax, &ifail);
  ...
  reldiff = MPRELDIFF(z, base, mantis, emin, emax, result_fmax, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_fmax, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of fmax(x,y):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      mapleX(verboseSummaryFile, y, "y", base, mantis);
      fprintf(verboseSummaryFile, " z := max(x,y);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_fmax, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp, *ymp, *result_fmax;
  int i, j, ifail, digs;
  char line[512];
  char *dec;
  REAL cycles, logba, logdc;

  /* Time fmax(x,y) with x x log-distributed in interval [a,b] and
