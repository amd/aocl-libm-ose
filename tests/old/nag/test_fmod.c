                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPFMOD(xmp, ymp, params, result_fmod, &ifail);
  ...
  reldiff = MPRELDIFF(z, base, mantis, emin, emax, result_fmod, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_fmod, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of fmod(x,y):\n");
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_fmod, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp, *ymp, *result_fmod;
  int i, j, ifail, digs, badSymmetry;
  char line[512];
  char *dec;
  REAL cycles, logba, logdc;

  /* Time fmod(x,y) x log-distributed in in the interval [a,b] and
