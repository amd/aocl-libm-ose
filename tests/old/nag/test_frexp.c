                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  REAL y, reldiff, ulps;
  REAL ulps1, ulps2;
  int *xmp, *result_frexp, *ymp, result_ipart_frexp, iexp;
  int ifail, digs;
  ...
  MPFREXP(xmp, result_frexp, &result_ipart_frexp, params, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_frexp, params,
                      &ulps1, &ifail);
  if(iexp == result_ipart_frexp)
    ulps2 = 0.0;
  else
    ulps2 = 10.0;
  ...
  dec = MP2DEC(result_frexp, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of frexp(x,iptr):\n");
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_frexp, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  REAL ulps1, ulps2;
  int *xmp, *result_frexp, result_ipart_frexp, iexp;
  int i, ifail, digs;
  char line[512];
  char *dec;
  REAL cycles, logba;

  /* Time frexp(x,iptr) in the interval [a,b] */
