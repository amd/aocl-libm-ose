                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPLDEXP(xmp, y, result_ldexp, params, &ifail);
  ...
  reldiff = MPRELDIFF(z, base, mantis, emin, emax, result_ldexp, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_ldexp, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of ldexp(x,y):\n");
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_ldexp, params, mantis, stdout);
    }
  ...
  if (maxreldiff > 0.0)
    {
      ...
      MPLDEXP(xmp, savy, result_ldexp, params, &ifail);
      digs = (int)(0.301 * 2 * mantis - 1);
      dec = MP2DEC(result_ldexp, params, digs, &ifail);
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time ldexp(x,y) with x x log-distributed in interval [a,b] and
