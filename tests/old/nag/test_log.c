                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPLOG(xmp, params, result_log, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of log(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := log(x);\n");
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time log(x) in the interval [a,b] */
