                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPLOG10(xmp, params, result_log, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_log, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_log, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of log10(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := log10(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_log, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time log10(x) in the interval [a,b] */
