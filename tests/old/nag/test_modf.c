                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  REAL ulps1, ulps2;
  int *xmp, *result_modf, *ymp, *result_ipart_modf;
  int ifail, digs;
  ...
  xmp = new_mp(params);
  ymp = new_mp(params);
  result_modf = new_mp(params);
  result_ipart_modf = new_mp(params);
  ...
  MPMODF(xmp, result_modf, result_ipart_modf, params, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_modf, params,
                      &ulps1, &ifail) +
            MPRELDIFF(iptr, base, mantis, emin, emax, result_ipart_modf, params,
                      &ulps2, &ifail);
  ulps = ulps1 + ulps2;
  ...
  dec = MP2DEC(result_modf, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of modf(x,iptr):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " iy := trunc(x);\n");
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_modf, params, mantis, stdout);
    }
                        ...
                        REAL *maxX, FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  REAL ulps1, ulps2;
  int *xmp, *result_modf, *result_ipart_modf;
  int i, ifail, digs;
  char *dec;
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time modf(x,iptr) in the interval [a,b] */
