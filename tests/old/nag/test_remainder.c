...
{
  REAL z, reldiff, ulps;
  int *xmp, *ymp, *result_remainder, *zmp;
  int ifail, digs;
  ...
  ymp = new_mp(params);
  zmp = new_mp(params);
  result_remainder = new_mp(params);
  ...
  MPREMAINDER(xmp, ymp, params, result_remainder, &ifail);
  ...
  reldiff = MPRELDIFF(z, base, mantis, emin, emax, result_remainder, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_remainder, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of remainder(x,y):\n");
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_remainder, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  int *xmp, *ymp, *result_remainder;
  int i, j, ifail, digs, badSymmetry;
  char line[512];
  char *dec;
  REAL cycles, logba, logdc;

  /* Time remainder(x,y) x log-distributed in in the interval [a,b] and
