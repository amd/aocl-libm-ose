                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPRINT(xmp, result_rint, params, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_rint, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_rint, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of rint(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := rint(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_rint, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time rint(x) in the interval [a,b] */
