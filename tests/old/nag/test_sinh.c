...
int main(int argc, char *argv[])
{
  ...
  if (!openResultsFiles(test_string,
  ...
  else
    {
      ...
      /* Choose elim such that sinh(2.0**elim) ~= large */
                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPSINH(xmp, params, result_sinh, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_sinh, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_sinh, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of sinh(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := sinh(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_sinh, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time sinh(x) in the interval [a,b] */
