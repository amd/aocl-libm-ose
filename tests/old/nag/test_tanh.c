...
int main(int argc, char *argv[])
{
  ...
  if (!openResultsFiles(test_string,
  ...
  else
    {
      ...
      /* Choose elim such that tanh(2.0**elim) ~= large */
                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPTANH(xmp, params, result_tanh, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_tanh, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_tanh, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of tanh(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := tanh(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_tanh, params, mantis, stdout);
    }
...
{
  REAL h, x = 0.0, y, reldiff, savx, savy, maxreldiff, ulps, maxulps;
  int *xmp, *result_tanh;
  ...
  /* Time tanh(x) in the interval [a,b] */
  ...
  if (verbose >= 1 && nTimes > 0)
    {
      sprintf(line, "Timing %d calls of %s for x = %30.20e\n", nTimes,
              FSPECSTR, x);
      fprintf(verboseSummaryFile, "%s", line);
      if (verbose >= 3)
        printf("%s", line);
