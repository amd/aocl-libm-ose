                  ...
                  FILE *summaryFile, FILE *verboseSummaryFile)
{
  ...
  MPTRUNC(xmp, result_trunc, params, &ifail);
  ...
  reldiff = MPRELDIFF(y, base, mantis, emin, emax, result_trunc, params,
                      &ulps, &ifail);
  ...
  dec = MP2DEC(result_trunc, params, digs, &ifail);
  ...
  if (verbose >= 2)
    {
      /* Output a small piece of Maple code to compute the result,
         as a check value. */
      fprintf(verboseSummaryFile, "# Maple code to check computation of trunc(x):\n");
      mapleX(verboseSummaryFile, x, "x", base, mantis);
      fprintf(verboseSummaryFile, " y := trunc(x);\n");
    }
  ...
  if (verbose >= 3)
    {
      ...
      printf("  expected =     %s\n", dec);
      MPSHOWBIN("           = ", result_trunc, params, mantis, stdout);
    }
                   ...
                   REAL *minTime, REAL *maxTime, REAL *maxTimeNormal)
{
  ...
  /* Time trunc(x) in the interval [a,b] */
