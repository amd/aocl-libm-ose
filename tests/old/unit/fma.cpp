...
#include "unit/fma.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(MAX(MAX(sizeof(double)/sizeof(unsigned long long int),sizeof(double)/sizeof(unsigned long long int)),sizeof(double)/sizeof(unsigned long long int)),sizeof(double)/sizeof(unsigned long long int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(fma)(P1.input, P2.input, P3.input);
        timer.Sample();
    }
};
void
RangeTest() {
    ...
    for (int i = 0; i < ntests; i += DATA_SIZE) {
        ...
        P2.V2 = InputVal<double, double>(min);
        P3.V2 = InputVal<double, double>(min);
    ...
    TestReport::Header("fma_rand.txt", 0);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(fma)(P1.input, P2.input, P3.input)    R.V3[j] = amd_ref_fma(P1.V3[j],P2.V3[j],P3.V3[j])");
