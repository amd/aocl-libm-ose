...
#include "unit/ldexpf.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(MAX(sizeof(float)/sizeof(unsigned int),sizeof(int)/sizeof(int)),sizeof(float)/sizeof(unsigned int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(ldexpf)(P1.input, P2.input);
        timer.Sample();
    }
};
void
RangeTest() {
    ...
    TestReport::Header("ldexpf_rand.txt", 0);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(ldexpf)(P1.input, P2.input)    R.V3[j] = ldexpf(P1.V3[j],P2.V3[j])");
