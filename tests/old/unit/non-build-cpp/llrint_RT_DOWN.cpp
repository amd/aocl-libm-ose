...
#include "unit/llrint_RT_DOWN.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(sizeof(double)/sizeof(unsigned long long int),sizeof(long long int)/sizeof(unsigned long long int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(llrint)(P1.input); timer.Sample();
    }
};
void
RangeTest() {
    ...
    TestReport::Header("llrint_RT_DOWN_rand.txt", 0);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(llrint)(P1.input)    R.V3[j] = lrint(P1.V3[j])");
