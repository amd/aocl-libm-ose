...
union R {
    unsigned long int V1[sizeof(long int) / sizeof(unsigned long int)];
    long int V3[sizeof(long int) / sizeof(long int)];
    long int V2;
};
union E {
...
#include "unit/lrintf_RT_DOWN.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(sizeof(float)/sizeof(unsigned int),sizeof(long int)/sizeof(unsigned long int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ParameterObj<float> P1;
    ParameterObj<long int> R;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(lrintf)(P1.input); timer.Sample();
    }
};
void
RangeTest() {
    ...
    TestReport::Header("lrintf_RT_DOWN_rand.txt", 0);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(lrintf)(P1.input)    R.V3[j] = lrintf(P1.V3[j])");
