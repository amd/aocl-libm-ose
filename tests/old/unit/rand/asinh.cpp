...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(asinh)(P1.input); timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("asinh_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(asinh)(P1.input)    R.V3[j] = asinh(P1.V3[j])");
