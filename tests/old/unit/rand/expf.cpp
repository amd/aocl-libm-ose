...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(expf)(P1.input); timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("expf_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(expf)(P1.input)    R.V3[j] = amd_ref_expf(P1.V3[j])");
