...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(expm1)(P1.input); timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("expm1_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(expm1)(P1.input)    R.V3[j] = amd_ref_expm1(P1.V3[j])");
