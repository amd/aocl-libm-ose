...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(fabs)(P1.input); timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("fabs_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(fabs)(P1.input)    R.V3[j] = fabs(P1.V3[j])");
