...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(fdimf)(P1.input, P2.input);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    for (int i = 0; i < size; i += DATA_SIZE) {
        ...
        for (unsigned int j = 0; j < sizeof(float) / sizeof(float); j++) {
            R.V3[j] = amd_ref_fdimf(P1.V3[j], P2.V3[j]);
        }
    ...
    TestReport::Header("fdimf_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(fdimf)(P1.input, P2.input)    R.V3[j] = amd_ref_fdimf(P1.V3[j],P2.V3[j])");
