...
#include "../unit/finite.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(sizeof(double)/sizeof(unsigned long long int),sizeof(int)/sizeof(unsigned int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(finite)(P1.input); timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("finite_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(finite)(P1.input)    R.V3[j] = finite(P1.V3[j])");
