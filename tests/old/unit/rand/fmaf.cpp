...
#include "../unit/fmaf.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(MAX(MAX(sizeof(float)/sizeof(unsigned int),sizeof(float)/sizeof(unsigned int)),sizeof(float)/sizeof(unsigned int)),sizeof(float)/sizeof(unsigned int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ParameterObj<float> P1;
    ParameterObj<float> P2;
    ParameterObj<float> P3;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(fmaf)(P1.input, P2.input, P3.input);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    for (int i = 0; i < size; i += DATA_SIZE) {
        ...
        P3.V2 = randomNumber<float, float>();
    ...
    TestReport::Header("fmaf_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(fmaf)(P1.input, P2.input, P3.input)    R.V3[j] = amd_ref_fmaf(P1.V3[j],P2.V3[j],P3.V3[j])");
