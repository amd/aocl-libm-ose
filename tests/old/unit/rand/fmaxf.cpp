...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(fmaxf)(P1.input, P2.input);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("fmaxf_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(fmaxf)(P1.input, P2.input)    R.V3[j] = fmaxf(P1.V3[j],P2.V3[j])");
