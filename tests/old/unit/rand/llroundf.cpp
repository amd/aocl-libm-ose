...
union R {
    ...
    long long int V3[sizeof(long long int) / sizeof(long long int)];
    long long int V2;
};
union E {
...
#include "../unit/llroundf.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(sizeof(float)/sizeof(unsigned int),sizeof(long long int)/sizeof(unsigned long long int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ParameterObj<float> P1;
    ParameterObj<long long int> R;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(llroundf)(P1.input); timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    for (int i = 0; i < size; i += DATA_SIZE) {
        ...
        R.V2 = randomNumber<long long int, long long int>();
    ...
    TestReport::Header("llroundf_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(llroundf)(P1.input)    R.V3[j] = llroundf(P1.V3[j])");
