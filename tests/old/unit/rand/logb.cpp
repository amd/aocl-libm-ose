...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(logb)(P1.input); timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("logb_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(logb)(P1.input)    R.V3[j] = amd_ref_logb(P1.V3[j])");
