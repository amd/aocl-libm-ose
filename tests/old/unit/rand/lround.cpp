...
#include "../unit/lround.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(sizeof(double)/sizeof(unsigned long long int),sizeof(long int)/sizeof(unsigned long int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(lround)(P1.input); timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("lround_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(lround)(P1.input)    R.V3[j] =lround(P1.V3[j])");
