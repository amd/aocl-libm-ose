...
#include "../unit/nearbyintf_RT_NEAR.h"
#include "FunHed.h"
...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(nearbyintf)(P1.input); timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("nearbyintf_RT_NEAR_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(nearbyintf)(P1.input)    R.V3[j] = nearbyintf(P1.V3[j])");
