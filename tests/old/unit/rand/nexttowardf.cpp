...
#include "../unit/nexttowardf.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(MAX(sizeof(float)/sizeof(unsigned int),sizeof(long double)/sizeof(long double)),sizeof(float)/sizeof(unsigned int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(nexttowardf)(P1.input, P2.input);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    for (int i = 0; i < size; i += DATA_SIZE) {
        ...
        P2.V2 = randomNumber<long double, long double>();
    ...
    TestReport::Header("nexttowardf_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(nexttowardf)(P1.input, P2.input)    R.V3[j] = nexttowardf(P1.V3[j], P2.V3[j])");
