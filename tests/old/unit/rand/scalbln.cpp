...
union P2 {
    unsigned long long int V1[sizeof(long long int) / sizeof(
                                unsigned long long int)];
    long int V3[sizeof(long long int) / sizeof(long int)];
    long long int V2;
};
union R {
    unsigned long long int V1[sizeof(double) / sizeof(unsigned long long int)];
...
#include "../unit/scalbln.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(MAX(sizeof(double)/sizeof(unsigned long long int),sizeof(long long int)/sizeof(unsigned long long int)),sizeof(double)/sizeof(unsigned long long int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ParameterObj<double> P1;
    ParameterObj<long int> P2;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(scalbln)(P1.input, P2.input);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    for (int i = 0; i < size; i += DATA_SIZE) {
        ...
        P2.V2 = randomNumber<long int, long int>();
    ...
    TestReport::Header("scalbln_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(scalbln)(P1.input, P2.input)    R.V3[j] = scalbln(P1.V3[j],P2.V3[j])");
