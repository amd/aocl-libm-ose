...
#include "../unit/scalbn.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(MAX(sizeof(double)/sizeof(unsigned long long int),sizeof(int)/sizeof(unsigned int)),sizeof(double)/sizeof(unsigned long long int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(scalbn)(P1.input, P2.input);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("scalbn_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(scalbn)(P1.input, P2.input)    R.V3[j] = scalbn(P1.V3[j],P2.V3[j])");
