...
#include "../unit/scalbnf.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(MAX(sizeof(float)/sizeof(unsigned int),sizeof(int)/sizeof(unsigned int)),sizeof(float)/sizeof(unsigned int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(scalbnf)(P1.input, P2.input);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("scalbnf_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(scalbnf)(P1.input, P2.input)    R.V3[j] = scalbnf(P1.V3[j],P2.V3[j])");
