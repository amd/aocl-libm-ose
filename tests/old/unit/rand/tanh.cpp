...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(tanh)(P1.input); timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("tanh_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(tanh)(P1.input)    R.V3[j] = amd_ref_tanh(P1.V3[j])");
