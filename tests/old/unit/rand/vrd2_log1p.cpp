...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(vrd2_log1p)(P1.input); timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("vrd2_log1p_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(vrd2_log1p)(P1.input)    R.V3[j]=amd_ref_log1p(P1.V3[j])");
