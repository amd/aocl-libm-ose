...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); FN_PROTOTYPE(vrda_expm1)(len.input, src.obtained, dst.obtained);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("vrda_expm1_rand.txt", seed);
    TestReport::FuncHeader("FN_PROTOTYPE(vrda_expm1)(len.input,src.obtained,dst.obtained)    dst.V3[0] = amd_ref_expm1(src.V3[0])");
    testdata.Report();
