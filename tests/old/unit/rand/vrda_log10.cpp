...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); FN_PROTOTYPE(vrda_log10)(len.input, src.obtained, dst.obtained);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("vrda_log10_rand.txt", seed);
    TestReport::FuncHeader("FN_PROTOTYPE(vrda_log10)(len.input,src.obtained,dst.obtained)    dst.V3[0] = amd_ref_log10(src.V3[0])");
    testdata.Report();
