...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(vrs4_exp10f)(P1.input); timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("vrs4_exp10f_rand.txt", seed);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(vrs4_exp10f)(P1.input)    R.V3[j]=amd_ref_exp10f(P1.V3[j])");
