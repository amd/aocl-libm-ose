...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); FN_PROTOTYPE(vrsa_cbrtf)(len.input, src.obtained, dst.obtained);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        dst.V3[0] = amd_ref_cbrtf(src.V3[0]);
    ...
    TestReport::Header("vrsa_cbrtf_rand.txt", seed);
    TestReport::FuncHeader("FN_PROTOTYPE(vrsa_cbrtf)(len.input,src.obtained,dst.obtained)    dst.V3[0] = amd_ref_cbrtf(src.V3[0])");
    testdata.Report();
