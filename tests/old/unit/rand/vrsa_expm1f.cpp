...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); FN_PROTOTYPE(vrsa_expm1f)(len.input, src.obtained, dst.obtained);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("vrsa_expm1f_rand.txt", seed);
    TestReport::FuncHeader("FN_PROTOTYPE(vrsa_expm1f)(len.input,src.obtained,dst.obtained)    dst.V3[0] = amd_ref_expm1f(src.V3[0])");
    testdata.Report();
