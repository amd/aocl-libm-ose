...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); FN_PROTOTYPE(vrsa_log1pf)(len.input, src.obtained, dst.obtained);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("vrsa_log1pf_rand.txt", seed);
    TestReport::FuncHeader("FN_PROTOTYPE(vrsa_log1pf)(len.input,src.obtained,dst.obtained)    dst.V3[0] = amd_ref_log1pf(src.V3[0])");
    testdata.Report();
