...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); FN_PROTOTYPE(vrsa_sinf)(len.input, src.obtained, dst.obtained);
        timer.Sample();
    }
};
void
RandomTest(int size, int seed) {
    ...
    TestReport::Header("vrsa_sinf_rand.txt", seed);
    TestReport::FuncHeader("FN_PROTOTYPE(vrsa_sinf)(len.input,src.obtained,dst.obtained)    dst.V3[0] = amd_ref_sinf(src.V3[0])");
    testdata.Report();
