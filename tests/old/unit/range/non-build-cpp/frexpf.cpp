...
#include "unit/frexpf.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(MAX(sizeof(float)/sizeof(unsigned int),sizeof(float)/sizeof(unsigned int)),sizeof(int)/sizeof(int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(frexpf)(P1.input, &P2.obtained);
        timer.Sample();
    }
};
void
RangeTest() {
    ...
    TestReport::Header("frexpf_rand.txt", 0);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(frexpf)(P1.input, &P2.obtained)    R.V3[j] = amd_ref_frexpf(P1.V3[j],&P2.V3[j])");
