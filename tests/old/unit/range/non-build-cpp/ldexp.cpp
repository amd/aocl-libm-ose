...
#include "unit/ldexp.h"
#include "FunHed.h"
#define DATA_SIZE (MAX(MAX(MAX(sizeof(double)/sizeof(unsigned long long int),sizeof(int)/sizeof(int)),sizeof(double)/sizeof(unsigned long long int)),sizeof(ExcFlags)/sizeof(ExcFlags)))
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(ldexp)(P1.input, P2.input);
        timer.Sample();
    }
};
void
RangeTest() {
    ...
    TestReport::Header("ldexp_rand.txt", 0);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(ldexp)(P1.input, P2.input)    R.V3[j] = ldexp(P1.V3[j],P2.V3[j])");
