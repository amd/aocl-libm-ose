...
class UnitTestData {
  public: Timer timer;
    ...
    Run() {
        ...
        timer.Start(); R.obtained = FN_PROTOTYPE(remainder)(P1.input, P2.input);
        timer.Sample();
    }
};
void
RangeTest() {
    ...
    for (int i = 0; i < ntests; i += DATA_SIZE) {
        ...
        for (unsigned int j = 0; j < sizeof(double) / sizeof(double); j++) {
            R.V3[j] = amd_ref_remainder(P1.V3[j], P2.V3[j]);
        }
    ...
    TestReport::Header("remainder_rand.txt", 0);
    TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(remainder)(P1.input, P2.input)    R.V3[j] = amd_ref_remainder(P1.V3[j],P2.V3[j])");
