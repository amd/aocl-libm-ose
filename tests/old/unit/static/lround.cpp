...
for(unsigned int j=0; j< sizeof(long int)/sizeof(unsigned long int); j++){
if((j+i) < LENGTH_OF(input))
R.V1[j] = input[i+j].R;
else
R.V1[j] = input[i].R;
...
TestReport::Header("lround.txt");
TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(lround)(P1.input)");
for(int j=0; j<k ; j++ ){
testdata[j].Report();
}
TestReport::Footer();
