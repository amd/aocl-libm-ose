...
timer.Start();R.obtained=FN_PROTOTYPE(nexttowardf)(P1.input, P2.input);timer.Sample();}};
int main(){
...
for(unsigned int j=0; j< sizeof(long double)/sizeof(long double); j++){
if((j+i) < LENGTH_OF(input))
...
TestReport::Header("nexttowardf.txt");
TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(nexttowardf)(P1.input, P2.input)");
for(int j=0; j<k ; j++ ){
testdata[j].Report();
}
TestReport::Footer();
