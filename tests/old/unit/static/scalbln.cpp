...
union P2 {
unsigned long long int V1[sizeof(long long int)/sizeof(unsigned long long int)];
long long int V2;
};
union R {
unsigned long long int V1[sizeof(double)/sizeof(unsigned long long int)];
...
timer.Start();R.obtained=FN_PROTOTYPE(scalbln)(P1.input, P2.input);timer.Sample();}};
int main(){
...
for(unsigned int j=0; j< sizeof(long long int)/sizeof(unsigned long long int); j++){
if((j+i) < LENGTH_OF(input))
...
TestReport::Header("scalbln.txt");
TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(scalbln)(P1.input, P2.input)");
for(int j=0; j<k ; j++ ){
testdata[j].Report();
}
TestReport::Footer();
