...
timer.Start();R.obtained=FN_PROTOTYPE(scalbn)(P1.input, P2.input);timer.Sample();}};
int main(){
...
for(unsigned int j=0; j< sizeof(int)/sizeof(unsigned int); j++){
if((j+i) < LENGTH_OF(input))
...
TestReport::Header("scalbn.txt");
TestReport::FuncHeader("R.obtained=FN_PROTOTYPE(scalbn)(P1.input, P2.input)");
for(int j=0; j<k ; j++ ){
testdata[j].Report();
}
TestReport::Footer();
