...
timer.Start();FN_PROTOTYPE(vrda_log1p)(len.input,src.obtained,dst.obtained);timer.Sample();}};
int main(){
...
testdata.SetName();
testdata.Run();
TestReport::Header("vrda_log1p.txt");
TestReport::FuncHeader("FN_PROTOTYPE(vrda_log1p)(len.input,src.obtained,dst.obtained)");
testdata.Report();
TestReport::Footer();
