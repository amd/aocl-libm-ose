...
#include "test_tanh.h"
#define __TEST_TANH_INTERNAL__                   /* needed to include tanh-test-data.h */
#include "test_tanh_data.h"
#include "tanh_accu_data.h"
...
#define _ZGVdN2v_tanh _ZGVbN2v_tanh
#define _ZGVdN4v_tanh _ZGVdN4v_tanh
#define _ZGVsN4v_tanhf _ZGVbN4v_tanhf
#define _ZGVsN8v_tanhf _ZGVdN8v_tanhf
#endif
*/

/*No vector double routines for tanh for now*/
/*
...
int test_tanh_conf_setup(struct libm_test *test)
{
    int ret = 0;
...
int test_tanh_special_setup(struct libm_test *test)
{
    int ret = 0;
    ...
    if(test_is_single_precision(test)) {
            ret = libm_test_conf_setup_f32(test,
                                          ...
                                          ARRAY_SIZE(test_tanhf_special_data));
    } else {
            ret = libm_test_conf_setup_f64(test,
                                          (struct libm_test_special_data_f64 *)
                                          test_tanh_special_data,
                                          ARRAY_SIZE(test_tanh_special_data));
	}
...
#if (LIBM_PROTOTYPE == PROTOTYPE_AOCL || LIBM_PROTOTYPE == PROTOTYPE_SVML)
static int
test_tanh_cb_v4s(struct libm_test *test, int j)
{
    struct libm_test_data *data = &test->test_data;
...
test_tanh_cb_v2d(struct libm_test *test, int j)
{
    ...
    __m128d op4 = LIBM_FUNC_VEC(d, 2, tanh)(ip2);
...
static int
test_tanh_cb_v4d(struct libm_test *test, int j)
{
    struct libm_test_data *data = &test->test_data;
    double *restrict ip1 = (double*)data->input1;
    double *restrict o = (double*)data->output;
...
static int
test_tanh_accu_run(struct libm_test *test)
{
    int ret = 0;
    if (test->conf->inp_range[0].start  == 0 ||
        test->conf->inp_range[0].stop   == 0)
    {
         int arr_sz = ARRAY_SIZE(tanh_accu_ranges);
         for (int i = 0; i < arr_sz; i++) {
                if ((tanh_accu_ranges[i].start == 0.0) && (tanh_accu_ranges[i].stop == 0.0))
                    break;

                ret = libm_generate_test_one_range(test, &tanh_accu_ranges[i]);

                if (ret) return ret;

                ret = test_tanh_verify(test, &test->result);
...
double test_tanhf_ulp(struct libm_test *test, int idx)
{
    float *buf = (float*)test->test_data.input1;
    float val = buf[idx];

    return alm_mp_tanhf(val);
}

double
test_tanh_ulp(struct libm_test *test, int idx)
{
    double *d = (double*)test->test_data.input1;
    double val = d[idx];

    if (test_is_single_precision(test)) {
        return test_tanhf_ulp(test, idx);
    }

    return alm_mp_tanh(val);
}

struct libm_test_funcs test_tanh_funcs[LIBM_FUNC_MAX] =
    {
     /*
      * Scalar functions
      */
     [LIBM_FUNC_S_S]  = {
                         ...
                         .accuracy     = { .setup = libm_test_accu_setup,
                                           .run   = test_tanh_accu_run,
                                           .ulp = {.func = test_tanhf_ulp}
                                         },


                         .special      = { .setup = test_tanh_special_setup,
                                           .run = libm_test_special,
                                           .verify = test_tanh_verify
                                          },


                         .conformance  = {.setup = test_tanh_conf_setup,
                                           .run   = libm_test_conf,
                                           .verify = test_tanh_verify
                                         },
     },
...
#define TANH_TEST_TYPES_ALL (TEST_TYPE_ACCU | TEST_TYPE_PERF |          \
...
int libm_test_init(struct libm_test_conf *c)
{
    ...
    if (!conf->test_types)
        conf->test_types = TANH_TEST_TYPES_ALL;
    ret = libm_tests_setup(c, test_tanh_funcs, &tanh_template);
    return ret;
}
