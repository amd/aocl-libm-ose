...
**Location:** `testsuite/src/` and `testsuite/src/include/`
...
### 7. **platform.h** - Cross-Platform Abstraction
...
### 9. **timing_wrapper.h** - Performance Measurement
...
**Usage**:
```bash
./testsuite <shim_library> <yaml_file> [--type|-t <accu|perf>]
```
...
### 2. **libm_process.cc** - Test Orchestration
...
### 12. **timing_wrapper.cc** - Platform-Specific Timing
...
### 2. Execution
```bash
./testsuite libaocl_shim.so sin_tests.yaml --type accu
```
