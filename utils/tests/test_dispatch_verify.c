...
{
    if (au_cpuid_arch_is_zen6(AU_CURRENT_CPU_NUM)) return "ZEN6";
    if (au_cpuid_arch_is_zen5(AU_CURRENT_CPU_NUM)) return "ZEN5";
    if (au_cpuid_arch_is_zen4(AU_CURRENT_CPU_NUM)) return "ZEN4";
    if (au_cpuid_arch_is_zen3(AU_CURRENT_CPU_NUM)) return "ZEN3";
    if (au_cpuid_arch_is_zen2(AU_CURRENT_CPU_NUM)) return "ZEN2";
    if (au_cpuid_arch_is_zen(AU_CURRENT_CPU_NUM))  return "ZEN";
    return "NON-AMD";
}
...
static dispatch_path_t get_expected_dispatch(void)
{
    ...
    if (au_cpuid_arch_is_zen3(AU_CURRENT_CPU_NUM)) {
        return DISPATCH_ZEN3;
    }
    if (au_cpuid_arch_is_zen2(AU_CURRENT_CPU_NUM)) {
        return DISPATCH_ZEN2;
    }
    if (au_cpuid_arch_is_zen(AU_CURRENT_CPU_NUM)) {
        return DISPATCH_ZEN;
    }
 ...
 * Uses "at-least" semantics: au_cpuid_arch_is_zenX() returns TRUE if CPU is zenX OR LATER
 */
...
static void test_dispatch_path_selection(void)
{
    ...
    if (strcmp(arch, "ZEN6") == 0) {
        ...
        /* Non-AMD */
        const char* avx512_both[] = {"avx512f", "avx512dq"};
        int has_both = au_cpuid_has_flags(AU_CURRENT_CPU_NUM, avx512_both, 2);
...
static void test_api_stability(void)
{
    ...
    int zen4_initial = au_cpuid_arch_is_zen4(AU_CURRENT_CPU_NUM);
    int avx512f_initial = au_cpuid_has_flags(AU_CURRENT_CPU_NUM, avx512f, 1);
    ...
    for (int i = 0; i < iterations; i++) {
        if (au_cpuid_arch_is_zen4(AU_CURRENT_CPU_NUM) != zen4_initial)
            zen4_stable = 0;
        if (au_cpuid_has_flags(AU_CURRENT_CPU_NUM, avx512f, 1) != avx512f_initial)
            avx512f_stable = 0;
    }
    
    CHECK(zen4_stable, "au_cpuid_arch_is_zen4 stable over 1000 calls");
    CHECK(avx512f_stable, "au_cpuid_has_flags stable over 1000 calls");
}
...
static void test_combined_flags(void)
{
    ...
    int has_avx512 = au_cpuid_has_flags(AU_CURRENT_CPU_NUM, avx512_flags, 2);
    ...
    if (has_avx512) {
        ...
        CHECK(au_cpuid_has_flags(AU_CURRENT_CPU_NUM, f, 1), 
              "Combined AVX512 includes AVX512F");
        CHECK(au_cpuid_has_flags(AU_CURRENT_CPU_NUM, dq, 1), 
        ...
        const char* f[] = {"avx512f"};
        const char* dq[] = {"avx512dq"};
        int has_f = au_cpuid_has_flags(AU_CURRENT_CPU_NUM, f, 1);
        int has_dq = au_cpuid_has_flags(AU_CURRENT_CPU_NUM, dq, 1);
    ...
    int has_unknown = au_cpuid_has_flags(AU_CURRENT_CPU_NUM, unknown_flags, 2);
...
static void print_cpu_report(void)
{
    ...
    printf("  Zen1:  %s\n", au_cpuid_arch_is_zen(AU_CURRENT_CPU_NUM) ? "YES" : "NO");
    printf("  Zen2:  %s\n", au_cpuid_arch_is_zen2(AU_CURRENT_CPU_NUM) ? "YES" : "NO");
    printf("  Zen3:  %s\n", au_cpuid_arch_is_zen3(AU_CURRENT_CPU_NUM) ? "YES" : "NO");
    printf("  Zen4:  %s\n", au_cpuid_arch_is_zen4(AU_CURRENT_CPU_NUM) ? "YES" : "NO");
    printf("  Zen5:  %s\n", au_cpuid_arch_is_zen5(AU_CURRENT_CPU_NUM) ? "YES" : "NO");
    printf("  Zen6:  %s\n", au_cpuid_arch_is_zen6(AU_CURRENT_CPU_NUM) ? "YES" : "NO");
    ...
    for (int i = 0; i < 2; i++) {
        printf("  %-10s: %s\n", names[i],
               au_cpuid_has_flags(AU_CURRENT_CPU_NUM, flags[i], 1) ? "YES" : "NO");
    }
